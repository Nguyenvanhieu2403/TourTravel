        ...
        if(empty($_POST["nameAccount"]) || empty($_POST["passwordAdmin"])){
            ...
            if ($result->num_rows > 0) {
                ...
                if(!$verify){
                    ...
                    if($status == 0){
                        header("Location: test_login_register.php"); // For employee
                    }
                    else if($status == 1){
                        header("Location: test_login_register.php"); // Wait to approve
                    }
                    else if($status == 2){
                        echo "Đăng nhập thành công";
                        header("Location: test_login_register.php"); // Change to home page (admin)
                    }
                    else{
                        header("Location: test_login_register.php");
