...
<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $checkAccount = $connect->prepare("SELECT * FROM `admin` WHERE `Email` = ? AND `Password` = ?");
        $checkAccount->bind_param("ss", $nameAccount, $passwordAdmin);
        $checkAccount->execute();

        // Get result from query 
        $result = $checkAccount->get_result();
        // Status = 0: Account has not been approved
        // Status = 1: Account has been approved
        // Status = 2: Supper admin's account
        ...
        if ($result->num_rows > 0) {
            ...
            if($status == 0){
                ...
                header("Location: test_login_register.html"); // Change to home page
            }
            else{
